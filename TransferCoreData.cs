...
// Script used to give a core the transfer (or no computation) behavior once it gets one input

public class TransferCoreData : MonoBehaviour
{
    ...
    public IEnumerator callAction(int val){
        ...
        if(coreData.returnValue != 0){
            ...
            coreData.storage = 2 * val;
            coreData.storage2 = val * -1;
        }
        else{
            coreData.returnValue = val;
